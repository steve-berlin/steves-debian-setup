...
- **Preflight checks build headers via `dpkg -s libpam0g-dev libxcb-xkb-dev`** (Zig links Ly against system PAM+XCB) and hard-fails on missing `git/curl/tar/sudo/systemctl`; also `dpkg -s libpam-systemd` (pam_systemd creates `/run/user/<uid>` — see PAM note below); arch guard maps only `x86_64`/`aarch64` to a Zig tarball name. **Under `--dry-run` every preflight miss downgrades to a `warning:` instead of an `exit 1`** (via a local `miss` helper) — a dry-run must still print the build/install plan it would run, and you'd typically dry-run on a box that hasn't installed the build toolchain yet.
- **Rewrites `/etc/pam.d/ly` with a Debian/MX PAM stack (`install_pam`), overwriting Ly's upstream default.** Upstream ships four `include login` lines; on Debian `/etc/pam.d/login` is getty-oriented and does **not** reliably run `pam_systemd` in Ly's session. Without pam_systemd, logind never creates `/run/user/<uid>` and `XDG_RUNTIME_DIR` stays root's `/run/user/0`, so Wayland sessions crash with **"Can't open Wayland socket"** / **"Unable to open lockfile"** and auth quirks surface as **"Can't authenticate user"**. The replacement is modeled on Debian's own vetted `sddm` PAM: `common-auth` for auth, a session that always reaches `common-session` (→ pam_systemd → runtime dir), `-`-prefixed keyring/kwallet lines (skipped if absent), and `pam_succeed_if user != root` (blocks root GUI login, per DM convention). Runs on **every** install/reinstall (idempotent — same bytes), so it repairs an already-installed box even when the version stamp skips the build. `--uninstall` deletes `/etc/pam.d/ly`. Takes effect at next login — the script does not restart `ly.service` (that would kill the live graphical session).
