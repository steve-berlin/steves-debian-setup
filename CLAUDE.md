...
Resolves every dep referenced by `~/.zshrc`: apt packages, oh-my-zsh + plugins, fzf, user toolchains (rustup, rbenv, atuin, starship, nvm, deno, bun, pyenv), Go + neovim tarballs, third-party installers (brave, waydroid), pip tools, flatpak + Organic Maps + Session messenger + SimpleX Chat + EarTag, Claude Code, caveman (Claude Code token-compression plugin), NordVPN, tmux config, helper scripts, XFCE + KDE keybindings, EasyEffects presets. **Step 0** (before any install) offers via a y/N prompt to delete `installers/discontinued/` — interactive-tty only; `--dry-run` and non-interactive/piped runs keep the scripts. **Step 8** seeds `~/.config/nvim/` from vendored `nvim-config/` via `rm -rf` + `cp -a` (was an upstream `git clone LazyVim/starter`); overwrites unconditionally. Sections 1–15 match `check-setup.sh`'s verification order; step 14 is a no-op stub (debloat moved to `debloat_scripts/`). The apt list includes `zathura` with the **poppler** render backend (`zathura-pdf-poppler`) plus `zathura-djvu`, the standalone `mupdf` viewer for EPUB/XPS/CBZ, and `ncdu` — see the pruning section on why there is no zathura mupdf backend to install. `shr` helper prints a short summary in `--dry-run` instead of the whole pipe for curl-piped installers + heredoc writes.

**yewtube + SimpleX Chat + EarTag.** **Step 9b** pip-installs `yewtube` (terminal YouTube client, the maintained mps-youtube fork) on its **own** pip line — it pins `httpx<0.28`, and folding it into step 9's `yt-dlp tldr platformio` line would let one resolver conflict take all four down. Its console entry point is **`yt`**, not `yewtube` (that's the name `check-setup.sh` step 8 probes). Runtime deps are already covered: `mpv` from the step-1 apt list, `yt-dlp` from step 9. **SimpleX Chat** (`chat.simplex.simplex`), **EarTag** (`app.drey.EarTag`) and **Telegram** (`org.telegram.desktop`) ride the existing step-10 flatpak block, same `--user` install as Organic Maps + Session; `check-setup.sh` step 9 probes each with `flatpak info --user`. Telegram comes from Flathub rather than Debian's `telegram-desktop` because the server enforces a minimum client version — a build that trails upstream far enough eventually refuses to connect outright, which a stable-release package will do. EarTag comes from Flathub, not Debian's `eartag` (trixie ships 0.6.5, upstream moves faster than the release cycle), keeping every GUI app on one update path.

**Package pruning (steps 1c/1d/1e)** runs right after the apt installs. **1c** purges `atmel-firmware` (chip firmware for Atmel at76c50x USB 802.11b dongles, dead weight on Intel-wifi boxes) — but only after `atmel_present()` clears: no device bound under `/sys/bus/usb/drivers/at76c50x_usb`, module not in `lsmod`, nothing matching in `lsusb`. Any hit keeps the package with a warning so a real device's firmware is never yanked. **1d** is **removed — do not reintroduce it.** It repointed `/bin/sh` at `zsh` and purged `dash`, and it bricked the box on 2026-08-02. `dash` *owns* `/bin/sh` on trixie (the old note here claimed `dpkg -S /bin/sh` finds no package — that only becomes true *after* a failed purge truncates `dash.list`; `dash.prerm` carries a `remove_divert /bin/sh`). dpkg therefore deletes `/bin/sh`, zsh symlink and all, partway through the purge, then execs `dash.postrm` — shebang `#!/bin/sh`, no interpreter, exit 1. Result: `dash` wedged `iH` half-installed, `/bin/sh` gone, every `#!/bin/sh` script on the system dead (including all dpkg maintainer scripts and the pending `debianutils`/`man-db`/`menu` triggers), and `set -e` killing utils.sh before its reassert line ran. No ordering of those two commands survives, because the delete happens *between* them inside dpkg. Recovery: `sudo ln -sf /usr/bin/bash /bin/sh` → `sudo apt-get install -y --reinstall dash` → `sudo dpkg --configure -a`. `check-setup.sh`'s matching `np dash` + `/bin/sh -> *zsh` assertions are gone too. **1e** purges `foliate` (GTK ebook reader) plus an `autoremove --purge` for the gjs/webkit stack it dragged in — `zathura` with the mupdf backend from step 1 already renders EPUB next to PDF/XPS/CBZ in one keyboard-driven viewer, so keeping both is duplicate weight. Both surviving steps are idempotent (skip when the package is already gone) and dry-run safe. `check-setup.sh` mirrors them (`np atmel-firmware` unless an at76 device is present; `np foliate`).

**There is no zathura mupdf backend on trixie — don't put one back in the apt line.** Neither `zathura-pdf-mupdf` (`packages.debian.org/trixie/zathura-pdf-mupdf` → "No such package") nor `zathura-mupdf` exists; `apt-cache pkgnames | grep mupdf` on MX 25 returns only `mupdf`, `mupdf-tools`, `libmupdf-dev`, `libmupdf25.1`, `python3-{mupdf,pymupdf}`. Debian dropped the zathura plugin, the engine stayed. Naming a nonexistent package here is not a soft failure: one bad name makes the whole single `apt-get install` line exit non-zero, `set -e` aborts, and **nothing in step 1 installs**.

So step 1 pairs **`zathura-pdf-poppler`** (PDF in zathura, alongside `zathura-djvu`) with the standalone **`mupdf`** viewer for EPUB/XPS/CBZ. Two keyboard-driven readers instead of one, which is the cost of the plugin being gone — but EPUB stays covered, which is what keeps step 1e's `foliate` purge honest. If you ever drop `mupdf` from the list, drop 1e in the same commit or the box ends up with no EPUB reader at all. Rebuilding the plugin from `libmupdf-dev` + `pwmt/zathura-pdf-mupdf` is possible but wants its own installer and a pinned mupdf — ABI churn against 25.1 is why Debian dropped it.

**caveman (step 10b′)** installs the [JuliusBrussee/caveman](https://github.com/JuliusBrussee/caveman) Claude Code plugin via its universal `curl … install.sh | bash -s -- --non-interactive`. Idempotency guard is the `~/.claude/.caveman-active` flag file the installer drops (not `have` — caveman is a plugin, not a PATH binary). Needs node ≥ 18: `source_nvm` sources nvm's `nvm.sh` (guarded against `set -u` fallout) to put node on PATH, then `nvm install --lts` (only when no `node` is already on PATH) provides one — utils.sh installs nvm but no default node version otherwise. Uninstall out-of-band with `utils.sh --uninstall-caveman` (runs `npx -y github:JuliusBrussee/caveman -- --uninstall`, then exits before the install flow) — a dedicated flag, not a general `--uninstall`, since utils.sh is a bulk installer with no whole-stack teardown.
