...
**Package pruning (steps 1c/1d/1e)** runs right after the apt installs. **1c** purges `atmel-firmware` (chip firmware for Atmel at76c50x USB 802.11b dongles, dead weight on Intel-wifi boxes) — but only after `atmel_present()` clears: no device bound under `/sys/bus/usb/drivers/at76c50x_usb`, module not in `lsmod`, nothing matching in `lsusb`. Any hit keeps the package with a warning so a real device's firmware is never yanked. **1d** repoints `/bin/sh` to `zsh` and purges `dash`. Order is load-bearing: `dash` is Essential and owns the `/bin/sh` symlink via debconf (`dpkg -S /bin/sh` finds no package), so the symlink is flipped to zsh *first* (else the purge strands `/bin/sh`), the purge passes `--allow-remove-essential` to clear the interactive "Yes, do as I say!" guard, and the symlink is reasserted afterward since dash's postrm may drop it. zsh runs `/bin/sh` in POSIX emulation — deliberate, non-default; revert with `apt-get install dash` + `dpkg-reconfigure dash`. **1e** purges `foliate` (GTK ebook reader) plus an `autoremove --purge` for the gjs/webkit stack it dragged in — `zathura` with the mupdf backend from step 1 already renders EPUB next to PDF/XPS/CBZ in one keyboard-driven viewer, so keeping both is duplicate weight. All three steps are idempotent (skip when the package is already gone) and dry-run safe. `check-setup.sh` mirrors all three (`np atmel-firmware` unless an at76 device is present; `np dash` + `/bin/sh -> *zsh`; `np foliate`).
